  ...
  if (!isConnected) {
    return (
      <button
        ...
        className={`inline-flex items-center justify-center gap-2 h-9 px-4 rounded-lg border border-[#3B82F6]/30 bg-[#3B82F6] hover:bg-[#2563EB] text-white text-xs font-semibold transition-colors whitespace-nowrap cursor-pointer disabled:opacity-50 ${className}`}
      >
        {isConnecting ? (
          <>
  ...
  return (
    <div className={`relative ${className}`}>
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
        className="inline-flex items-center gap-2.5 h-9 px-3.5 rounded-lg border border-white/[0.08] bg-[#161B26] hover:bg-[#1C2331] text-xs transition-colors cursor-pointer group"
      >
